...
const CreateEmployee = () => {
        ...
        if (formData.id !== null) {
            ...
            setEmployees(prev => 
                prev.map((employee, index) => 
                    index === formData.id ? { ...formData, id: index } : employee
                )
            );
        } else {
            // Add new employee
            const newEmployee = {
                id: employees.length, // Use the length as the unique ID
                ...formData,
            };
    ...
    const handleEdit = (employee, index) => {
        setFormData({ ...employee, id: index }); // Set id as the index for editing
    };
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className='btn btn-warning'>{formData.id !== null ? 'Update Employee' : 'Add Employee'}</button>
            </form>
                    ...
                    {filteredEmployees.length > 0 ? (
                        filteredEmployees.map((employee, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    <button onClick={() => handleEdit(employee, index)} className='btn btn-info'>Edit</button>
